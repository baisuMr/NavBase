...
**免认证**：`<img>` 标签无法携带 Basic Auth 头，该端点在认证门中放行；仅返回公开网站图标（域名由调用方提供），不含任何用户数据。
...
- 响应经图片魔数校验（PNG/GIF/JPEG/BMP/ICO/WebP），错误页等非图片内容视为失败；单图上限 512KB；**SVG 一律拒收**（直接访问该 URL 时恶意 SVG 可在站点源下执行脚本）
