...
NavBase 是个人自用的开源网址导航管理平台（MIT），用于替代浏览器书签功能。前端 SPA 与 API 同部署于 Cloudflare Workers（Static Assets + Worker 脚本），数据存 D1，Basic Auth 单用户认证。

**项目已进入维护期**：功能基本满足需求，日常只有 bug 修复、体验小改和小功能增加（想法记录在 `TODO.md`）。**严禁把项目做复杂**：

- 技术栈已锁死：Vue 3 + JavaScript（不用 TypeScript）、纯 CSS（不用 Tailwind 等框架）、Cloudflare Workers + D1。不引入新依赖、新框架、新架构，确有必要须先征得用户同意
- 不做企业化功能（多用户、权限体系、插件系统、深色模式等），不做用户没提的需求，不做「将来可能用到」的预留设计
- 一切改动保持最小：能小改不大改，不顺手重构、不重命名、不调整无关代码
- 需求不明确或存在多种做法时，先向用户提问，再按回答动手

## 常用命令

完整脚本见 `package.json`；部署与 D1 运维步骤见 `README.md`。
...
调用链分层：视图 → composables → Pinia stores → `src/api/` 封装 → Worker（`worker/index.js` 认证门 + 显式路由表，处理函数在 `worker/routes/`）。前端不直接 fetch，一律走 `src/api/` 封装（自动附带 Basic 认证头）；例外：登录接口由 auth store 直连 fetch（需避开 401 跳转逻辑）、settings store 无 composable 封装由视图直接引用。端点契约见 `API.md`。

**不新增透传型 composable**：现有 useBookmarks/useCategories/useAuth 属历史透传层（纯转发 store），新代码直接 `useXxxStore()` 引用 store，不再加转发层。

## 核心设计（既定行为，勿随意更改）

- **单页面应用**: 主页面 Home.vue 同时具备浏览和管理功能（搜索/常用站点内联进 Home.vue，时钟+日期行在 HeroClock.vue 组件——秒级 tick 只重渲染该子树），第一屏（大字时钟 + 日期/农历/周数、搜索框含引擎下拉与 Alt+K 键帽、手动固定的常用站点快捷卡片、滚动提示）占满视口高度、第二屏书签库最小视口高度
...
- **UI/样式/视觉类修改**：一律由用户人工测试验证，**禁止调用浏览器自动化工具（Playwright、Chrome DevTools MCP 等）做验证**
- **简单小问题的修复**：同样采用人工测试验证，但完成后必须给出测试与验收步骤（操作路径 + 预期结果），交由用户执行确认
