...
NavManager（默认站点名「栞记」）是一个自用的网址导航管理平台，用于替代浏览器书签功能。前端 SPA 与 API 同部署于 Cloudflare Pages，数据存 D1，Basic Auth 单用户认证。
...
- **自动获取图标**: 书签的 favicon 通过 `/api/favicon/:domain` 获取（并发探测 favicon.im、DuckDuckGo 等源取最先成功者），前端必须走带认证头的 api 封装
